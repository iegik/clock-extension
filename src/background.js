(async () => {
    ...
    const syncStyleProperty = (element) => ([attr, prop]) => {
        if(!element) return;
    ...
    if (typeOfClock === 'digital') {
        ...
        clock = new DigitalClock({ element, color, fontFamily, fontWeight, fontSize, showSeconds, showMilliseconds, showShadow, color, size })

        // hack to center not monospaced font
        clock.element.style.width = `${clock.element.offsetWidth}px`
    } else {
        if (root) root.innerHTML = /==$/.test(analogueTheme) // check for Base64
            ? atob(analogueTheme)
        ...
        clock = new AnalogueClock({ element, hoursArrow, minutesArrow, secondsArrow, color, showSeconds, showShadow, color, size })
    }
